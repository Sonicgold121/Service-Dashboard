...
def find_row_in_gsheet(worksheet, rma_to_find, sn_to_find, headers):
    ...
    try:
        ...
        return -1 # Indicates header not found
...
def get_archived_reports_from_gsheet():
    ...
    try:
        ...
        try:
            ...
            st.error(f"Archive sheet '{ARCHIVE_SHEET_NAME}' not found. Please create it with headers: Report Date, Needs Estimate Creation, Needs Shipping, Needs Reminder.") # Updated headers
            return []
        ...
        for rec in records:
            try:
                ...
                needs_reminder_str = rec.get('Needs Reminder', '[]') # New
                ...
                needs_reminder_list = json.loads(needs_reminder_str) if needs_reminder_str and needs_reminder_str.strip() else [] # New

                archived_reports.append({
                    "date": rec.get('Report Date'),
                    "needs_estimate_creation": needs_est_list,
                    "needs_shipping": needs_ship_list,
                    "needs_reminder": needs_reminder_list # New
                })
...
def save_report_to_gsheet_archive(report_data):
    ...
    try:
        ...
        try:
            ...
            st.error(f"Archive sheet '{ARCHIVE_SHEET_NAME}' not found. Create it with headers: Report Date, Needs Estimate Creation, Needs Shipping, Needs Reminder."); return False # Updated headers
        ...
        needs_reminder_str = json.dumps(report_data['needs_reminder']) # New
        archive_ws.append_row([report_data['date'], needs_estimate_str, needs_shipping_str, needs_reminder_str]) # New
...
def create_excel_report_bytes(report_data):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer: 
        ...
        sheets_data = {
            ...
            "Needs Reminder": report_data.get("needs_reminder", []), # New
...
def display_formatted_report(report_data, source="Newly Generated", report_key_suffix=""):
    ...
    st.markdown(f"**📞 Needs Reminder (Estimate Sent 2 days prior to {report_data['date']}):**") # New
    if report_data.get('needs_reminder'): # Check if key exists
...
if 'todays_tasks_for_eod_report' not in st.session_state: st.session_state.todays_tasks_for_eod_report = None # New
if 'end_of_day_summary_report' not in st.session_state: st.session_state.end_of_day_summary_report = None # New
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.todays_tasks_for_eod_report = None # Clear on refresh
    st.session_state.end_of_day_summary_report = None # Clear on refresh
    st.rerun() 
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    ...
    st.session_state.selected_archived_report_to_display = None 
    st.session_state.custom_report_to_display = None 
    st.session_state.end_of_day_summary_report = None # Clear EOD report
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
             ...
             # Check if today's task list needs to be (re)generated if last_gen was also today
             if last_gen_date_from_archive == today:
                 st.session_state.todays_tasks_for_eod_report = generate_single_day_report_content(data_df, today)
                 st.sidebar.info("Today's task list for EOD report re-confirmed.")

        else:
            while current_date_to_report <= today:
                report_data = generate_single_day_report_content(data_df, current_date_to_report)
                if current_date_to_report == today: # Store today's tasks for EOD report
            ...
            if reports_generated_this_run:
                ...
                if not st.session_state.todays_tasks_for_eod_report and last_gen_date_from_archive == today: # Ensure it's set if archive was up to date
                     st.session_state.todays_tasks_for_eod_report = generate_single_day_report_content(data_df, today)

            st.rerun() 

# --- NEW: End of Day Summary Report Button ---
st.sidebar.markdown("---")
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    if 'todays_tasks_for_eod_report' not in st.session_state or st.session_state.todays_tasks_for_eod_report is None:
        ...
        summary_data = {"date": date.today().strftime("%Y-%m-%d"), "estimate_tasks": [], "shipping_tasks": []}
        todays_tasks = st.session_state.todays_tasks_for_eod_report

        # Check Estimate Creation Tasks
        for task in todays_tasks.get("needs_estimate_creation", []):
            rma, sn = task.get("RMA"), task.get("S/N")
            record = data_df[(data_df['RMA'] == rma) & (data_df['S/N'] == sn)]
            status = "Pending"
            if not record.empty and record.iloc[0]['Estimate Sent To Email'].lower() != 'n/a':
                status = "Completed"
            summary_data["estimate_tasks"].append({"RMA": rma, "S/N": sn, "Status": status, "Original Task": f"Create/Send Estimate (Est. Complete: {task.get('Est. Complete Date')})"})
        
        # Check Shipping Tasks
        for task in todays_tasks.get("needs_shipping", []):
            rma, sn = task.get("RMA"), task.get("S/N")
            record = data_df[(data_df['RMA'] == rma) & (data_df['S/N'] == sn)]
            status = "Pending"
            if not record.empty and record.iloc[0]['Shipped'].lower() == 'yes':
                status = "Completed"
            summary_data["shipping_tasks"].append({"RMA": rma, "S/N": sn, "Status": status, "Original Task": f"Ship Item (QA'd on {todays_tasks['date']})"})
        
        st.session_state.end_of_day_summary_report = summary_data
        st.sidebar.success("End of Day Summary Generated.")
        st.rerun()

# --- Display Newly Generated Reports / Selected Archive / Custom Report / EOD Report---
...
if st.session_state.get('end_of_day_summary_report'): # Display EOD Summary
    ...
    if eod_summary['estimate_tasks']:
        ...
        st.dataframe(eod_est_df, use_container_width=True)
    ...
    with pd.ExcelWriter(eod_output, engine='xlsxwriter') as writer:
        ...
        if eod_summary['estimate_tasks']:
            df_eod_est = pd.DataFrame(eod_summary['estimate_tasks'])
            df_eod_est.to_excel(writer, sheet_name="EOD Estimate Tasks", startrow=2, index=False, header=False)
            ws1 = writer.sheets["EOD Estimate Tasks"]
            ws1.merge_range(0,0,0, len(df_eod_est.columns)-1 if len(df_eod_est.columns)>0 else 0, f"EOD - Estimate Tasks - {eod_summary['date']}", title_format)
            ws1.set_row(0,30)
            for cn, val in enumerate(df_eod_est.columns.values): ws1.write(2, cn, val, header_format)
            for rn in range(3, len(df_eod_est)+3):
                for cn_idx in range(len(df_eod_est.columns)): ws1.write(rn, cn_idx, df_eod_est.iloc[rn-3, cn_idx], cell_format)
            for i, col in enumerate(df_eod_est.columns):
                col_len = max(df_eod_est[col].astype(str).map(len).max(), len(col)) + 2 if not df_eod_est[col].empty else len(col)+2
                ws1.set_column(i,i,col_len)
        else:
            ws1 = writer.book.add_worksheet("EOD Estimate Tasks")
            ws1.merge_range(0,0,0,2, f"EOD - Estimate Tasks - {eod_summary['date']}", title_format)
            ws1.write(2,0, "No estimate tasks for today.", cell_format)

        if eod_summary['shipping_tasks']:
            df_eod_ship = pd.DataFrame(eod_summary['shipping_tasks'])
            df_eod_ship.to_excel(writer, sheet_name="EOD Shipping Tasks", startrow=2, index=False, header=False)
            ws2 = writer.sheets["EOD Shipping Tasks"]
            ws2.merge_range(0,0,0, len(df_eod_ship.columns)-1 if len(df_eod_ship.columns)>0 else 0, f"EOD - Shipping Tasks - {eod_summary['date']}", title_format)
            ws2.set_row(0,30)
            for cn, val in enumerate(df_eod_ship.columns.values): ws2.write(2, cn, val, header_format)
            for rn in range(3, len(df_eod_ship)+3):
                for cn_idx in range(len(df_eod_ship.columns)): ws2.write(rn, cn_idx, df_eod_ship.iloc[rn-3, cn_idx], cell_format)
            for i, col in enumerate(df_eod_ship.columns):
                col_len = max(df_eod_ship[col].astype(str).map(len).max(), len(col)) + 2 if not df_eod_ship[col].empty else len(col)+2
                ws2.set_column(i,i,col_len)
        else:
            ws2 = writer.book.add_worksheet("EOD Shipping Tasks")
            ws2.merge_range(0,0,0,2, f"EOD - Shipping Tasks - {eod_summary['date']}", title_format)
            ws2.write(2,0, "No shipping tasks for today.", cell_format)
