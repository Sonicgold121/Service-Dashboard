...
if not data_df.empty:
    ...
    for display_name, col_name in date_filter_columns_to_filter.items():
        # --- Setup widget parameters from original data_df ---
        min_val_for_widget_setup = None
        max_val_for_widget_setup = None
        can_setup_widget = False

        if col_name in data_df.columns and pd.api.types.is_datetime64_any_dtype(data_df[col_name]):
            original_col_for_widget_params = data_df[col_name].dropna() # Use dropna() before min/max
        ...
        if can_setup_widget:
            current_date_range_selection = st.sidebar.date_input(
                ...
                value=(min_val_for_widget_setup, max_val_for_widget_setup), # Default to full range of original data
                min_value=min_val_for_widget_setup,
                max_value=max_val_for_widget_setup,
                key=f"date_range_{col_name}"
            )

            # --- Apply this selection to filter `filtered_df` ---
            if col_name in filtered_df.columns and pd.api.types.is_datetime64_any_dtype(filtered_df[col_name]):
                if current_date_range_selection and len(current_date_range_selection) == 2:
                    ...
                    condition = (
                        (filtered_df[col_name] >= start_datetime_selected) &
                        (filtered_df[col_name] <= end_datetime_selected) &
                        (filtered_df[col_name].notna()) 
                    )
                    filtered_df = filtered_df[condition]
        # else: No valid date range in original data_df for this column, so don't show the filter.
            # st.sidebar.info(f"Date filter for '{display_name}' not available due to missing/invalid data.")
