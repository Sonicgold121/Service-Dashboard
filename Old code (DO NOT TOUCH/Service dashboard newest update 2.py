...
# Initialize session state for first load flags and refresh counter
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.refresh_counter += 1 # Increment counter to change widget keys
    st.rerun() 
...
if not data_df.empty:
    ...
    for col_name, search_label in [('RMA', "RMA"), ('S/N', "S/N"), ('Part Number', "Part Number"), ('SPC Code', "SPC Code")]:
        if col_name in filtered_df.columns:
            search_term = st.sidebar.text_input(f"Search by {search_label}", key=f"search_{col_name}_{st.session_state.refresh_counter}") # Add refresh counter to key
    ...
    for display_name, col_name in status_columns_to_filter.items():
        if col_name in data_df.columns: 
            ...
            selected_status = st.sidebar.selectbox(f"Filter by {display_name}", unique_values, 
                                                   key=f"select_{col_name}_{st.session_state.refresh_counter}", # Add refresh counter
                                                   index=default_index)
    ...
    for display_name, col_name in date_filter_columns_to_filter.items():
        ...
        if can_setup_widget:
            current_date_range_selection = st.sidebar.date_input(f"Filter by {display_name}", 
                ...
                key=f"date_range_{col_name}_{st.session_state.refresh_counter}") # Key includes refresh_counter
