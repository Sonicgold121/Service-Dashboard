...
# import urllib.parse # No longer needed for BC links in table
...
# --- Constants for Business Central Link (No longer used for table, but kept if needed elsewhere) ---
...
def load_data_from_google_sheet(
    ...
    try:
        ...
        if headers != EXPECTED_COLUMN_ORDER:
            # Check if the sheet has fewer columns than expected, which can happen.
            # Create a mapping from existing headers to expected order to handle this.
            # This is a more robust way if the Google Sheet headers might not be perfect.
            temp_df = pd.DataFrame(data_rows, columns=headers)
            df = pd.DataFrame(columns=EXPECTED_COLUMN_ORDER) # Start with an empty df with correct columns
        ...
        # This loop is now redundant if the above logic correctly initializes missing columns
        # but kept for safety to ensure all EXPECTED_COLUMN_ORDER columns exist.
        for col in EXPECTED_COLUMN_ORDER:
        ...
        df = df[cols_to_use] # Ensure correct order
...
# --- REMOVED: Function to generate HTML table with clickable RMA ---
...
if not data_df.empty:
    ...
    # --- MODIFIED: Reverted to st.dataframe for display ---
    if not filtered_df.empty:
        display_cols = [col for col in EXPECTED_COLUMN_ORDER if col in filtered_df.columns]
        # For st.dataframe, it's often better to pass dtypes as they are,
        # especially datetimes, rather than converting all to string beforehand.
        # Streamlit handles display of NaT and None reasonably well.
