...
@st.cache_data(ttl=60) # Cache for 1 min to avoid rapid re-reads during report generation
def get_archived_reports_from_gsheet():
    ...
    try:
        ...
        records = archive_ws.get_all_records() # Assumes headers: Report Date, Needs Estimate Creation, Needs Shipping
        archived_reports = []
        for rec in records:
            try:
                # Parse the stringified lists back into Python lists of dicts
                needs_est_str = rec.get('Needs Estimate Creation', '[]')
                needs_ship_str = rec.get('Needs Shipping', '[]')
                
                # Handle potential empty strings from sheet before parsing JSON
                ...
                st.warning(f"Could not parse archived report data for date {rec.get('Report Date')}: {e}. Skipping this entry.")
            except Exception as e_gen:
                 st.warning(f"Unexpected error processing archived report for date {rec.get('Report Date')}: {e_gen}. Skipping this entry.")
...
def get_last_report_date_from_archive(archived_reports):
    if not archived_reports:
        return date.today() - timedelta(days=1) # Default if no reports found
    try:
        # Assuming reports are sorted descending by date, first one is latest
...
def save_report_to_gsheet_archive(report_data):
    ...
    try:
        ...
        try:
            ...
            st.error(f"Archive sheet '{ARCHIVE_SHEET_NAME}' not found. Please create it first with headers: Report Date, Needs Estimate Creation, Needs Shipping.")
            return False

        # Check if report for this date already exists
        existing_dates = archive_ws.col_values(1)[1:] # Get all dates, skip header
        if report_data['date'] in existing_dates:
            # st.info(f"Report for {report_data['date']} already exists in Google Sheet archive.") # Less verbose
            return False # Indicate it wasn't newly saved

        # Convert lists of dicts to JSON strings for storage in a single cell
        ...
        get_archived_reports_from_gsheet.clear() # Clear cache so next read gets the new one
        return True
    except Exception as e:
        st.error(f"Error saving report to Google Sheet archive: {type(e).__name__} - {e}")
        return False
...
def create_excel_report_bytes(report_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if not df_needs_estimate.empty: df_needs_estimate.to_excel(writer, index=False, sheet_name='Needs Estimate Creation')
        else: pd.DataFrame().to_excel(writer, index=False, sheet_name='Needs Estimate Creation')
        df_needs_shipping = pd.DataFrame(report_data.get("needs_shipping", []))
        if not df_needs_shipping.empty: df_needs_shipping.to_excel(writer, index=False, sheet_name='Needs Shipping')
        else: pd.DataFrame().to_excel(writer, index=False, sheet_name='Needs Shipping')
...
# Removed session state for last_report_generated_for_date and archived_reports, will use GSheet
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    get_archived_reports_from_gsheet.clear() # Clear archive cache too
...
archived_reports_gsheet = get_archived_reports_from_gsheet() # Load archive once per run
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
            ...
            while current_date_to_report <= today:
                ...
                if save_report_to_gsheet_archive(report_data): # Save to GSheet
                    reports_generated_this_run.append(report_data)
                    get_archived_reports_from_gsheet.clear() # Update archive cache
...
if not data_df.empty:
    ...
    st.subheader("🗂️ Daily Status Report Archive") # Changed from "Session Only"
    if archived_reports_gsheet: # Use data loaded from GSheet
        ...
        if available_months:
            ...
            if reports_to_list:
                for i, report_data_item in enumerate(reports_to_list): 
                    ...
                    with col2:
                        if st.button("View/Reprint", key=f"view_archive_{report_data_item['date']}_{i}"):
                            st.session_state.selected_archived_report_to_display = report_data_item
                            st.session_state.newly_generated_reports_to_display = []; st.rerun()
                    # Removed quick look expander for clarity, full view on button click
