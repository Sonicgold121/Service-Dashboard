...
def save_report_to_gsheet_archive(report_data, archive_sheet_name_to_save, archive_headers_to_check):
    ...
    try:
        ...
        try:
            ...
            archive_ws = spreadsheet.add_worksheet(title=archive_sheet_name_to_save, rows="100", cols=str(len(archive_headers_to_check))) # Start with more rows
        ...
        if archive_sheet_name_to_save == ARCHIVE_SHEET_NAME:
            if report_data['date'] in existing_dates: return False # Don't duplicate daily task lists
            row_to_append.extend([
             ...
             # For EOD, we might overwrite if it already exists for the day
            if report_data['date'] in existing_dates:
                try:
                    cell = archive_ws.find(report_data['date'])
                    row_number_to_update = cell.row
                    # Prepare list of lists for update_cells
                    update_values = [
                        report_data['date'],
                    ...
                    # Construct cell_list for batch update
                    cell_list_to_update = archive_ws.range(row_number_to_update, 1, row_number_to_update, len(update_values))
                    for i, cell_to_update in enumerate(cell_list_to_update):
                        cell_to_update.value = update_values[i]
                    archive_ws.update_cells(cell_list_to_update)
                    ...
                    return True # Indicate update
                     ...
                     pass # Should not happen if date is in existing_dates, but proceed to append if so
            # If not updating existing, then append
            row_to_append.extend([
                json.dumps(report_data['estimate_tasks']),
                json.dumps(report_data['reminder_tasks']),
                json.dumps(report_data['shipping_tasks']),
...
if st.session_state.get('end_of_day_summary_report'): 
    ...
    with pd.ExcelWriter(eod_output, engine='xlsxwriter') as writer:
        # ... (Excel writing logic for EOD summary remains the same) ...
        workbook = writer.book
        header_format = workbook.add_format({'bold': True, 'text_wrap': False, 'valign': 'top', 'fg_color': '#D7E4BC', 'border': 1, 'align': 'center'})
        cell_format = workbook.add_format({'border': 1})
        title_format = workbook.add_format({'bold': True, 'font_size': 14, 'align': 'center', 'valign': 'vcenter'})
