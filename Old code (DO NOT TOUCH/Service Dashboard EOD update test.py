    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
             ...
             if last_gen_date_from_archive == today:
                 # Ensure todays_tasks_for_eod_report is set if it's today and report already exists
                 if st.session_state.get('todays_tasks_date') != today or st.session_state.get('todays_tasks_for_eod_report') is None:
                    ...
                    st.session_state.todays_tasks_date = today # Track the date of these tasks
                    st.sidebar.info("Today's task list for EOD report re-confirmed/set.")


        else:
            while current_date_to_report <= today:
                report_data = generate_single_day_report_content(data_df, current_date_to_report)
                # Store today's tasks for EOD report only if it's the first time for today or not set
                if current_date_to_report == today:
                    if st.session_state.get('todays_tasks_date') != today or st.session_state.get('todays_tasks_for_eod_report') is None:
            ...
            if reports_generated_this_run:
                ...
                # Ensure todays_tasks_for_eod_report is set if it's today and report already exists
                if not st.session_state.get('todays_tasks_for_eod_report') and last_gen_date_from_archive == today:
                     st.session_state.todays_tasks_for_eod_report = generate_single_day_report_content(data_df, today)
                     st.session_state.todays_tasks_date = today
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    if 'todays_tasks_for_eod_report' not in st.session_state or st.session_state.todays_tasks_for_eod_report is None:
    ...
    elif current_data_for_eod.empty: # Check the freshly loaded data
        st.sidebar.warning("No data loaded to generate end-of-day summary.")
    else:
        summary_data = {"date": date.today().strftime("%Y-%m-%d"), "estimate_tasks": [], "shipping_tasks": [], "reminder_tasks": []} # Added reminder_tasks
        ...
        for task in todays_tasks.get("needs_estimate_creation", []):
            ...
            record = current_data_for_eod[(current_data_for_eod['RMA'] == rma) & (current_data_for_eod['S/N'] == sn)]
            status = "Pending"
        ...
        for task in todays_tasks.get("needs_reminder", []):
            rma, sn = task.get("RMA"), task.get("S/N")
            record = current_data_for_eod[(current_data_for_eod['RMA'] == rma) & (current_data_for_eod['S/N'] == sn)]
            status = "Pending"
            if not record.empty and record.iloc[0]['Reminder Completed'].lower() == 'yes':
                status = "Completed"
            summary_data["reminder_tasks"].append({"RMA": rma, "S/N": sn, "Status": status, "Original Task": f"Send Reminder (Est. Sent: {task.get('Estimate Sent Time')})"})
...
if st.session_state.get('end_of_day_summary_report'): # Display EOD Summary
    ...
    st.markdown("**Reminder Task Summary:**") # New
    if eod_summary.get('reminder_tasks'):
        eod_rem_df = pd.DataFrame(eod_summary['reminder_tasks'])
        st.dataframe(eod_rem_df, use_container_width=True)
    ...
    with pd.ExcelWriter(eod_output, engine='xlsxwriter') as writer:
        ...
        eod_sheets_data = {
            ...
            "EOD Reminder Tasks": eod_summary.get("reminder_tasks", []), # New
