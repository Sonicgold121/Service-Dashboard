...
def generate_single_day_report_content(df, report_date_obj):
    """Generates structured report content for a single specific date."""
    report_content = {
        "date": report_date_obj.strftime("%Y-%m-%d"), 
        "needs_shipping": [], # List of dicts: [{'RMA': ..., 'S/N': ...}]
        "needs_estimate_creation": [] # List of dicts
    }
    # Needs Shipping: QA Approved on report_date_obj, not Shipped
    shipping_df = df[
...
def create_excel_report_bytes(report_data):
    """Creates an Excel file in bytes from the structured report data."""
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_needs_estimate = pd.DataFrame(report_data.get("needs_estimate_creation", []))
        if not df_needs_estimate.empty:
            df_needs_estimate.to_excel(writer, index=False, sheet_name='Needs Estimate Creation')
        else: # Create empty sheet if no data
            pd.DataFrame().to_excel(writer, index=False, sheet_name='Needs Estimate Creation')

        df_needs_shipping = pd.DataFrame(report_data.get("needs_shipping", []))
        if not df_needs_shipping.empty:
            df_needs_shipping.to_excel(writer, index=False, sheet_name='Needs Shipping')
        else: # Create empty sheet if no data
            pd.DataFrame().to_excel(writer, index=False, sheet_name='Needs Shipping')
    return output.getvalue()

def display_formatted_report(report_data, source="Newly Generated", report_key_suffix=""):
    """Displays a single daily status report and provides a download button."""
    ...
    if report_data['needs_estimate_creation']:
        for item in report_data['needs_estimate_creation']:
            # Original string format: f"RMA: {item['RMA']}, S/N: {item['S/N']} (Est. Complete: {item['Est. Complete Date']})"
    ...
    if report_data['needs_shipping']:
        for item in report_data['needs_shipping']:
            # Original string format: f"RMA: {item['RMA']}, S/N: {item['S/N']}"
            st.markdown(f"- RMA: {item.get('RMA', 'N/A')}, S/N: {item.get('S/N', 'N/A')}")
    else:
        st.info("None for this category.")

    # Add download button for this report
    excel_bytes = create_excel_report_bytes(report_data)
    st.download_button(
        label=f"Download Report for {report_data['date']} (Excel)",
        data=excel_bytes,
        file_name=f"Daily_Status_Report_{report_data['date']}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key=f"download_report_{report_data['date']}_{report_key_suffix}" # Unique key
    )
    st.markdown("---")
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
            ...
            while current_date_to_report <= today:
                ...
                if save_report_to_archive(report_data): reports_generated_this_run.append(report_data)
                if current_date_to_report == today: set_last_report_date(today); break 
            ...
            elif last_gen_date == today : st.sidebar.info("Daily report for today already generated or no new days to report.")
...
if not data_df.empty:
    ...
    if st.session_state.archived_reports:
        ...
        if available_months:
            ...
            if reports_to_list:
                for i, report_data_item in enumerate(reports_to_list): # Renamed report_data to report_data_item
                    ...
                    with col2:
                        if st.button("View/Reprint", key=f"view_archive_{report_data_item['date']}_{i}"):
                            ...
                            st.session_state.newly_generated_reports_to_display = []; st.rerun()
                    with st.expander(f"Quick look: {report_data_item['date']}", expanded=False):
                         display_formatted_report(report_data_item, source="Archived (Quick Look)", report_key_suffix=f"ql_{i}")
