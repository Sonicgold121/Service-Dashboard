...
EXPECTED_COLUMN_ORDER = [
    ...
    "Reminder Completed", "Reminder Completed Time", "Reminder Contact Method", # New
        ...
        for col in EXPECTED_COLUMN_ORDER:
            if col in temp_df.columns:
                df[col] = temp_df[col] 
                 ...
                 elif col == "Estimate Sent To Email" or col == "Reminder Contact Method": df[col] = "N/A" # Specific default
                 else: df[col] = "N/A" 
...
def _update_reminder_in_sheet(worksheet, headers, rma, sn, reminder_date_obj, contact_method): # Added contact_method
    reminder_status_col_name = "Reminder Completed"; reminder_time_col_name = "Reminder Completed Time"
    reminder_method_col_name = "Reminder Contact Method" # New
    try:
        ...
        reminder_method_col_idx = headers.index(reminder_method_col_name) + 1 # New
    ...
    if row_to_update != -1:
        ...
        updates = [
            ...
            {'range': gspread.utils.rowcol_to_a1(row_to_update, reminder_method_col_idx), 'values': [[contact_method]]} # New
        ]
        if update_gsheet_cells(worksheet, updates):
...
def update_reminder_details_in_gsheet(rma, sn, reminder_date_obj, contact_method): # Added contact_method
...
def identify_overdue_estimates(df, days_threshold=3):
    required_cols = ['Estimate Complete Time', 'Estimate Complete', 'Estimate Approved', 'RMA',
                     'Estimate Sent To Email', 'Estimate Sent Time', 
                     'Reminder Completed', 'Reminder Completed Time', 'Reminder Contact Method'] # Added Reminder Contact Method
           ...
           pd.notna(row['Estimate Complete Time']):
            if (now - row['Estimate Complete Time']).days > days_threshold:
                ...
                overdue_items.append({
                    ...
                    'Reminder Contact Method': row.get('Reminder Contact Method', 'N/A'), # New
...
def identify_overdue_reminders(df, days_threshold=2):
    required_cols = ['Estimate Sent Time', 'Estimate Sent To Email', 'Reminder Completed', 'RMA']
    if df.empty or not all(col in df.columns for col in required_cols): return pd.DataFrame()
...
def get_archived_reports_from_gsheet():
    ...
    try:
        ...
        try:
            ...
            st.error(f"Archive sheet '{ARCHIVE_SHEET_NAME}' not found. Please create it with headers: Report Date, Needs Estimate Creation, Needs Shipping.")
            return []
...
def save_report_to_gsheet_archive(report_data):
    ...
    try:
        ...
        try:
            ...
            st.error(f"Archive sheet '{ARCHIVE_SHEET_NAME}' not found. Create it with headers: Report Date, Needs Estimate Creation, Needs Shipping."); return False
        ...
        archive_ws.append_row([report_data['date'], needs_estimate_str, needs_shipping_str])
...
def create_excel_report_bytes(report_data):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer: 
        ...
        df_needs_estimate = pd.DataFrame(report_data.get("needs_estimate_creation", []))
        sheet1_name = 'Needs Estimate Creation'
        if not df_needs_estimate.empty:
            df_needs_estimate.to_excel(writer, sheet_name=sheet1_name, startrow=2, index=False, header=False)
            worksheet1 = writer.sheets[sheet1_name]
            worksheet1.merge_range(0, 0, 0, len(df_needs_estimate.columns)-1 if len(df_needs_estimate.columns)>0 else 0, f"Needs Estimate Creation - Report Date: {report_data['date']}", title_format)
            worksheet1.set_row(0, 30) 
            for col_num, value in enumerate(df_needs_estimate.columns.values): worksheet1.write(2, col_num, value, header_format)
            for row_num in range(3, len(df_needs_estimate) + 3): 
                for col_num in range(len(df_needs_estimate.columns)):
                    worksheet1.write(row_num, col_num, df_needs_estimate.iloc[row_num-3, col_num], cell_format) 
            for i, col in enumerate(df_needs_estimate.columns):
                column_len = max(df_needs_estimate[col].astype(str).map(len).max(), len(col)) + 2 if not df_needs_estimate[col].empty else len(col) + 2
                worksheet1.set_column(i, i, column_len)
        else: 
            worksheet1 = writer.book.add_worksheet(sheet1_name) 
            worksheet1.merge_range(0, 0, 0, 2, f"Needs Estimate Creation - Report Date: {report_data['date']}", title_format)
            worksheet1.write(2,0, "No items for this category.", cell_format)

        df_needs_shipping = pd.DataFrame(report_data.get("needs_shipping", []))
        sheet2_name = 'Needs Shipping'
        if not df_needs_shipping.empty:
            df_needs_shipping.to_excel(writer, sheet_name=sheet2_name, startrow=2, index=False, header=False)
            worksheet2 = writer.sheets[sheet2_name]
            worksheet2.merge_range(0, 0, 0, len(df_needs_shipping.columns)-1 if len(df_needs_shipping.columns)>0 else 0, f"Needs Shipping - Report Date: {report_data['date']}", title_format)
            worksheet2.set_row(0, 30) 
            for col_num, value in enumerate(df_needs_shipping.columns.values): worksheet2.write(2, col_num, value, header_format)
            for row_num in range(3, len(df_needs_shipping) + 3):
                for col_num in range(len(df_needs_shipping.columns)):
                    worksheet2.write(row_num, col_num, df_needs_shipping.iloc[row_num-3, col_num], cell_format)
            for i, col in enumerate(df_needs_shipping.columns):
                column_len = max(df_needs_shipping[col].astype(str).map(len).max(), len(col)) + 2 if not df_needs_shipping[col].empty else len(col) + 2
                worksheet2.set_column(i, i, column_len)
        else:
            worksheet2 = writer.book.add_worksheet(sheet2_name) 
            worksheet2.merge_range(0,0,0,2, f"Needs Shipping - Report Date: {report_data['date']}", title_format)
            worksheet2.write(2,0, "No items for this category.", cell_format)
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
            ...
            elif last_gen_date_from_archive >= today : st.sidebar.info("Daily report for today already in archive or no new days to report.")
            st.rerun() 
...
custom_report_date = st.date_input("Select Date for Custom Report:", value=date.today(), key=f"custom_report_date_picker_{st.session_state.refresh_counter}")
if st.button("Generate Report for Selected Date", key=f"gen_custom_report_btn_{st.session_state.refresh_counter}"):
    if data_df.empty:
        st.warning("No data loaded to generate a custom report.")
    elif custom_report_date:
        st.session_state.custom_report_to_display = generate_single_day_report_content(data_df, custom_report_date)
        st.session_state.newly_generated_reports_to_display = [] 
        st.session_state.selected_archived_report_to_display = None
        st.rerun() 
    else:
        st.warning("Please select a date for the custom report.")
...
if not data_df.empty:
    ...
    st.subheader("🗣️ Overdue Reminders Report (Estimate Sent > 2 Days, Reminder Not Done)") # New Report Section
                ...
                for i, report_data_item in enumerate(reports_to_list): 
                    ...
                    with col2:
                        if st.button("View/Download Report", key=f"view_archive_{report_data_item['date']}_{i}"): 
                            ...
                            st.session_state.newly_generated_reports_to_display = []; st.session_state.custom_report_to_display = None; st.rerun()
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Sent To Email', 'Reminder Completed']):
        ...
        if not eligible_reminder_df.empty:
            ...
            if selected_item_reminder and selected_item_reminder != "Select item...":
                ...
                # New: Selectbox for Reminder Contact Method
                ...
                if st.sidebar.button("Mark Reminder Completed", key=f"mark_reminder_button_{st.session_state.refresh_counter}"):
                    if rma_reminder and sn_reminder and reminder_date_val and reminder_contact_method: # Ensure contact method is selected
