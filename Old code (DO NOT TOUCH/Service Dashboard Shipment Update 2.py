...
def get_last_report_date():
    if 'last_report_generated_for_date' not in st.session_state:
        st.session_state.last_report_generated_for_date = date.today() - timedelta(days=1) # Default to ensure today's report runs
...
def generate_single_day_report_content(df, report_date_obj):
    ...
    estimate_df = df[
        ...
        (df['Estimate Sent To Email'].astype(str).str.lower() == 'n/a') & # Check if estimate not yet sent
...
def save_report_to_archive(report_data):
    if 'archived_reports' not in st.session_state: st.session_state.archived_reports = []
    ...
    if report_data['date'] not in existing_report_dates:
        ...
        st.session_state.archived_reports.sort(key=lambda r: r['date'], reverse=True) 
        return True
    return False

# --- NEW: Function to display a formatted report ---
def display_formatted_report(report_data, source="Newly Generated"):
    """Displays a single daily status report in a formatted way."""
    ...
    if report_data['needs_estimate_creation']:
        ...
        st.info("None for this category.")
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.newly_generated_reports_to_display = [] # Clear newly generated on full refresh
    st.session_state.selected_archived_report_to_display = None # Clear selected archive on full refresh
    st.rerun() 
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    st.session_state.newly_generated_reports_to_display = [] # Clear previous new reports
    st.session_state.selected_archived_report_to_display = None # Clear any selected archived report
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
             st.sidebar.info("Reports are up to date for today.")
        else:
            while current_date_to_report <= today:
                # st.sidebar.write(f"Generating report for: {current_date_to_report.strftime('%Y-%m-%d')}...") # Less verbose
                report_data = generate_single_day_report_content(data_df, current_date_to_report)
                
                if save_report_to_archive(report_data): 
                     reports_generated_this_run.append(report_data)
                # else:
                     # st.sidebar.info(f"Report for {current_date_to_report.strftime('%Y-%m-%d')} already in session archive.")

                if current_date_to_report == today: 
                    set_last_report_date(today) 
                    break 
                current_date_to_report += timedelta(days=1)
                if (current_date_to_report - (last_gen_date + timedelta(days=1))).days > 30 : 
            ...
            if reports_generated_this_run:
                ...
                st.sidebar.success(f"{len(reports_generated_this_run)} daily report(s) generated.")
            elif last_gen_date == today : # If no new reports were generated because we are up to date
                 st.sidebar.info("Daily report for today already generated or no new days to report.")
            set_last_report_date(today) 
            st.rerun() # Rerun to display newly generated reports

# --- Display Newly Generated Reports (if any) ---
if st.session_state.newly_generated_reports_to_display:
    st.markdown("---")
    st.subheader("✨ Newly Generated Daily Status Report(s)")
    for report in st.session_state.newly_generated_reports_to_display:
        display_formatted_report(report, source="Newly Generated")
    # Option to clear this display after viewing
    if st.button("Clear Newly Generated Reports View", key="clear_new_reports"):
        st.session_state.newly_generated_reports_to_display = []
        st.rerun()
    st.markdown("---")

# --- Display Selected Archived Report (if any) ---
if st.session_state.selected_archived_report_to_display:
    st.markdown("---")
    display_formatted_report(st.session_state.selected_archived_report_to_display, source="Archived")
...
if not data_df.empty:
    ...
    if st.session_state.archived_reports:
        ...
        if available_months:
            ...
            reports_to_list = [r for r in st.session_state.archived_reports if selected_month_archive == "All" or datetime.strptime(r['date'], "%Y-%m-%d").strftime("%Y-%m") == selected_month_archive]

            if reports_to_list:
                for i, report_data in enumerate(reports_to_list):
                    col1, col2 = st.columns([3,1])
                    with col1:
                        st.markdown(f"**Report for: {report_data['date']}**")
                    with col2:
                        if st.button("View/Reprint", key=f"view_archive_{report_data['date']}_{i}"):
                            st.session_state.selected_archived_report_to_display = report_data
                            st.session_state.newly_generated_reports_to_display = [] # Clear other display
                            st.rerun()
                    # Optionally, show a brief summary or just the button
                    with st.expander(f"Quick look: {report_data['date']}", expanded=False):
                         display_formatted_report(report_data, source="Archived (Quick Look)")


            else: st.info(f"No reports found for {selected_month_archive} in this session's archive.")
        else: st.info("No archived reports available in this session yet.")
    else: st.info("No archived reports available in this session yet. Generate reports using the button in the sidebar.")
    st.markdown("---")
