...
def identify_overdue_estimates(df, days_threshold=3):
    required_cols = ['Estimate Complete Time', 'Estimate Complete', 'Estimate Approved', 'RMA', 'S/N', 'SPC Code', 
                     'Estimate Sent To Email', 'Shipped'] # Removed other reminder/sent cols as they will be N/A
    ...
    for _, row in df_copy.iterrows():
        ...
        is_not_sent = str(row.get('Estimate Sent To Email', 'N/A')).lower() == 'n/a' # New condition
        complete_time = row['Estimate Complete Time']
        
        # Condition changed to only show if estimate has NOT been sent
        if is_estimate_complete and is_not_approved and is_not_shipped and is_not_sent and pd.notna(complete_time): 
            if (now - complete_time).days > days_threshold:
                ...
                overdue_items.append({
                    ...
                    'Days Overdue for Sending': (now - complete_time).days, # Changed label
...
def identify_overdue_reminders(df, days_threshold=2):
    required_cols = ['Estimate Sent Time', 'Estimate Sent To Email', 'Reminder Completed', 'RMA', 'S/N', 'SPC Code', 'Reminder Contact Method',
                     'Estimate Approved'] # Added Estimate Approved for filtering
    if df.empty or not all(col in df.columns for col in required_cols): return pd.DataFrame()
    ...
    for _, row in df_copy.iterrows():
        ...
        # Condition changed to include is_estimate_not_approved
...
if not data_df.empty:
    ...
    if not overdue_estimates_df.empty:
        st.warning("The following estimates were completed more than 3 days ago, have not been sent, and are still pending approval:")
        overdue_estimates_display_cols = ['RMA', 'S/N', 'SPC Code', 'Estimate Complete Time', 'Days Overdue for Sending']
        st.dataframe(overdue_estimates_df[overdue_estimates_display_cols], use_container_width=True)
    else: st.success("✅ No estimates are currently overdue for sending or approval.")
    st.markdown("---")
    ...
    if not overdue_reminders_df.empty:
        st.info("The following items had estimates sent >2 days ago and are pending a reminder:")
        ...
        st.success("✅ No items are currently overdue for a reminder (that are still pending approval).")
    st.markdown("---")
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Sent To Email', 'Reminder Completed', 'Estimate Approved']): 
        eligible_reminder_df = data_df[ (data_df['Estimate Sent To Email'].astype(str).str.lower() != 'n/a') & (data_df['Reminder Completed'].astype(str).str.lower().isin(['no', 'n/a'])) & (data_df['Estimate Approved'].astype(str).str.lower().isin(['no', 'n/a'])) ] # Added this condition
