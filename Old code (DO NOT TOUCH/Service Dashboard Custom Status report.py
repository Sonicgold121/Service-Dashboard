...
def find_row_in_gsheet(worksheet, rma_to_find, sn_to_find, headers):
    ...
    # The first row of all_data_values is the header, data starts at index 1.
    # enumerate(all_data_values[1:], start=2) means 'i' will be the 1-based sheet row number.
    for i, row_values in enumerate(all_data_values[1:], start=2): 
        ...
        # Robust comparison: convert to string, strip whitespace, and compare lowercased versions.
        if rma_val_from_sheet is not None and sn_val_from_sheet is not None:
            # Ensure inputs are also treated as strings for comparison
    ...
    return -1 # Indicates row not found
...
def gsheet_update_wrapper(update_function, *args):
    try:
        ...
        worksheet = spreadsheet.get_worksheet(WORKSHEET_INDEX) # Main data sheet
...
def generate_single_day_report_content(df, report_date_obj):
    ...
    # Needs Shipping: Estimate Complete, Estimate Approved, QA Approved are all YES, Shipped is NO/N/A, and QA Approved Time is on report_date_obj
    shipping_df = df[
...
if 'custom_report_to_display' not in st.session_state: st.session_state.custom_report_to_display = None # New state for custom report
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.custom_report_to_display = None # Clear custom report on refresh
    st.rerun() 
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    st.session_state.newly_generated_reports_to_display = [] 
    st.session_state.selected_archived_report_to_display = None 
    st.session_state.custom_report_to_display = None # Clear custom report
    if data_df.empty:
...
# --- NEW: Custom Date Status Report Section ---
st.markdown("---")
...
if st.button("Generate Report for Selected Date", key=f"gen_custom_report_btn_{st.session_state.refresh_counter}"):
    if data_df.empty:
        ...
        st.session_state.newly_generated_reports_to_display = [] # Clear other displays
        st.session_state.selected_archived_report_to_display = None
        st.rerun() # Rerun to display the custom report
    else:
