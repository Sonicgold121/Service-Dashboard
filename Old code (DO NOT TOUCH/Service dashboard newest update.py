...
def load_data_from_google_sheet(
    ...
    try:
        ...
        # Create DataFrame with headers from sheet first
        temp_df = pd.DataFrame(data_rows, columns=headers_from_sheet)
        
        # Initialize the final DataFrame with EXPECTED_COLUMN_ORDER
        ...
        for col in EXPECTED_COLUMN_ORDER:
            if col in temp_df.columns:
                df[col] = temp_df[col] 
                 ...
                 else: df[col] = "N/A" # Default for other text fields like Description, Comments etc.
        ...
        for col in string_cols_to_process:
            if col in df.columns:
                ...
                # Replace various forms of empty/NA with a consistent "N/A" for string/status columns
                # For status columns, "No" is the active default, "N/A" means not applicable/not set yet.
                # Let's be careful: if a status is "No", it should remain "No".
                # If it's empty or "nan", it should become "N/A" (or "No" if it's a status column not yet set)
                if col in ALL_STATUS_COLUMNS:
                     df[col] = df[col].replace(['', 'nan', 'None', 'NaN', 'NONE', None, 'NaT'], 'No') # Default to "No" for statuses
                elif col == "Estimate Sent To Email":
                     df[col] = df[col].replace(['', 'nan', 'None', 'NaN', 'NONE', None, 'NaT'], 'N/A')
                else: # For other descriptive text columns
                     df[col] = df[col].replace(['', 'nan', 'None', 'NaN', 'NONE', None, 'NaT'], 'N/A')


        for col in ALL_TIME_COLUMNS:
            if col in df.columns:
                # Before converting to datetime, if it's 'N/A' (from above) or other non-date strings, make it None
...
def find_row_in_gsheet(worksheet, rma_to_find, sn_to_find, headers):
    ...
    all_data_values = worksheet.get_all_values() # get_all_values includes headers as first list
    for i, row_values in enumerate(all_data_values[1:], start=2): # Data rows start from sheet row 2
        ...
        if rma_val == rma_to_find and sn_val == sn_to_find:
            return i # Return 1-based sheet row index
    return -1
...
def gsheet_update_wrapper(update_function, *args):
    """Wraps gsheet update logic to handle connection and row finding."""
    try:
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/spreadsheets",
                 "https://www.googleapis.com/auth/drive.file", "https://www.googleapis.com/auth/drive"]
        creds = ServiceAccountCredentials.from_json_keyfile_name(CREDS_FILE, scope)
...
def _update_estimate_sent_in_sheet(worksheet, headers, rma, sn, sent_to_email, sent_date_obj):
    sent_time_col_name = "Estimate Sent Time"
    sent_email_col_name = "Estimate Sent To Email"
    try:
        sent_time_col_idx = headers.index(sent_time_col_name) + 1
        sent_email_col_idx = headers.index(sent_email_col_name) + 1
    except ValueError:
        st.error(f"'{sent_time_col_name}' or '{sent_email_col_name}' not in sheet headers.")
        return False
...
def _update_reminder_in_sheet(worksheet, headers, rma, sn, reminder_date_obj):
    reminder_status_col_name = "Reminder Completed"
    reminder_time_col_name = "Reminder Completed Time"
    try:
        ...
        st.error(f"'{reminder_status_col_name}' or '{reminder_time_col_name}' not in sheet headers.")
        return False
    ...
    if row_to_update != -1:
        ...
        if update_gsheet_cells(worksheet, updates):
            st.success(f"Reminder for RMA {rma}, S/N {sn} marked as completed on {reminder_date_obj.strftime('%Y-%m-%d')}.")
            return True
...
def _update_shipped_in_sheet(worksheet, headers, rma, sn, shipped_date_obj):
    shipped_status_col_name = "Shipped"
    shipped_time_col_name = "Shipped Time"
    try:
        shipped_status_col_idx = headers.index(shipped_status_col_name) + 1
        shipped_time_col_idx = headers.index(shipped_time_col_name) + 1
    except ValueError:
        st.error(f"'{shipped_status_col_name}' or '{shipped_time_col_name}' not in sheet headers.")
        return False
    ...
    if row_to_update != -1:
        ...
        if update_gsheet_cells(worksheet, updates):
            st.success(f"Successfully marked RMA {rma}, S/N {sn} as shipped on {shipped_date_obj.strftime('%Y-%m-%d')}.")
            return True
    else: st.error(f"Record with RMA {rma} and S/N {sn} not found in Google Sheet. Update failed.")
    return False

# Public facing update functions using the wrapper
...
def update_shipped_status_in_gsheet(rma, sn, shipped_date_obj): # Renamed from original
...
def display_kpis(df):
    ...
    kpi_cols = {
        "Est. Complete": 'Estimate Complete', "Est. Approved": 'Estimate Approved',
        "Est. Sent": 'Estimate Sent To Email', # Count if not N/A
        "Reminders Done": 'Reminder Completed', "QA Approved": 'QA Approved',
        "Units Shipped": 'Shipped'
    }
    kpi_values = {"Total Records": total_records}
    for label, col_name in kpi_cols.items():
        if col_name in df.columns:
            if col_name == 'Estimate Sent To Email':
                kpi_values[label] = df[df[col_name].astype(str).str.lower() != 'n/a'].shape[0]
            else:
                kpi_values[label] = df[df[col_name].astype(str).str.lower() == 'yes'].shape[0]
        ...
        if str(row.get('Estimate Complete', 'N/A')).lower() == 'yes' and \
           ...
           pd.notna(row['Estimate Complete Time']):
            if (now - row['Estimate Complete Time']).days > days_threshold:
                ...
                overdue_items.append({'RMA': rma_value, 'S/N': row.get('S/N', 'N/A'),
                    'Estimate Complete Time': row['Estimate Complete Time'].strftime('%Y-%m-%d'),
                    'Days Pending Approval': (now - row['Estimate Complete Time']).days, BC_LINK_COL_NAME: bc_url })
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.first_load_complete = False; st.rerun() 
...
if not data_df.empty:
    ...
    if not overdue_estimates_df.empty:
        ...
        if BC_LINK_COL_NAME not in overdue_estimates_df.columns: overdue_estimates_df[BC_LINK_COL_NAME] = None 
    ...
    filtered_df = data_df.copy() # Start with a full copy for filtering
    ...
    for display_name, col_name in status_columns_to_filter.items():
        if col_name in data_df.columns: # Check existence in original data_df for options
            unique_values = ['All'] + sorted(list(set(val for val in data_df[col_name].astype(str).unique() if val and val.strip() != ''))) # Use data_df for options
            if 'N/A' in data_df[col_name].astype(str).unique() and 'N/A' not in unique_values: unique_values.insert(1, "N/A") 
            if 'No' in data_df[col_name].astype(str).unique() and 'No' not in unique_values: unique_values.insert(1, "No") # Ensure "No" is an option if present
            ...
            selected_status = st.sidebar.selectbox(f"Filter by {display_name}", unique_values, key=f"select_{col_name}", index=default_index)
            if selected_status != "All": 
                if col_name in filtered_df.columns: # Apply filter on filtered_df
    ...
    for display_name, col_name in date_filter_columns_to_filter.items():
        ...
        if can_setup_widget:
            default_date_value = tuple() if not st.session_state.first_load_complete else (min_val_for_widget_setup, max_val_for_widget_setup)
            if not st.session_state.first_load_complete: # Make it blank on first load
                default_date_value = [] # For st.date_input, empty list or tuple makes it blank

            current_date_range_selection = st.sidebar.date_input(f"Filter by {display_name}", value=default_date_value, 
                min_value=min_val_for_widget_setup, max_value=max_val_for_widget_setup, key=f"date_range_{col_name}")
            
            if current_date_range_selection and len(current_date_range_selection) == 2: # User has selected a range
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Approved', 'Estimate Sent To Email']):
        ...
        if not eligible_estimate_sent_df.empty:
            ...
            selected_item_est_sent = st.sidebar.selectbox("Item (Approved, Not Sent)", options, index=0, key="est_sent_item_selector")
            if selected_item_est_sent and selected_item_est_sent != "Select item...":
                ...
                sent_to_email = st.sidebar.text_input("Sent To Email Address", key="est_sent_email_input")
                sent_date_val = st.sidebar.date_input("Estimate Sent Date", value=date.today(), key="est_sent_date_input")
                if st.sidebar.button("Mark Estimate Sent", key="mark_est_sent_button"):
                    if rma_est_sent and sn_est_sent and sent_to_email and sent_date_val:
                        if "@" not in sent_to_email or "." not in sent_to_email: 
                            ...
                            if success:
                                load_data_from_google_sheet.clear(); st.session_state.data_df = load_data_from_google_sheet()
                                st.session_state.first_load_complete = False; st.sidebar.success("Estimate sent details updated!"); st.rerun()
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Sent To Email', 'Reminder Completed']):
        ...
        if not eligible_reminder_df.empty:
            ...
            selected_item_reminder = st.sidebar.selectbox("Item (Estimate Sent, Reminder Pending)", options, index=0, key="reminder_item_selector")
            if selected_item_reminder and selected_item_reminder != "Select item...":
                ...
                reminder_date_val = st.sidebar.date_input("Reminder Date", value=date.today(), key="reminder_date_input")
                if st.sidebar.button("Mark Reminder Completed", key="mark_reminder_button"):
                    if rma_reminder and sn_reminder and reminder_date_val:
                        ...
                        if success:
                            load_data_from_google_sheet.clear(); st.session_state.data_df = load_data_from_google_sheet()
                            st.session_state.first_load_complete = False; st.sidebar.success("Reminder details updated!"); st.rerun()
                ...
                try:
                    ...
                    if st.sidebar.button("Mark as Shipped", key="mark_shipped_button"):
                        if rma_to_update and sn_to_update and shipped_date_val:
                            ...
                            if success:
                                load_data_from_google_sheet.clear(); st.session_state.data_df = load_data_from_google_sheet() 
                                st.session_state.first_load_complete = False; st.sidebar.success("Update successful! Data refreshed."); st.rerun() 
