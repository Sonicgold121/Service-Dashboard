...
def identify_overdue_estimates(df, days_threshold=3):
    required_cols = ['Estimate Complete Time', 'Estimate Complete', 'Estimate Approved', 'RMA', 'S/N', 'SPC Code', # Added SPC Code
                     'Estimate Sent To Email', 'Estimate Sent Time', 
                     'Reminder Completed', 'Reminder Completed Time', 'Reminder Contact Method'] 
           ...
           pd.notna(row['Estimate Complete Time']):
            if (now - row['Estimate Complete Time']).days > days_threshold:
                ...
                overdue_items.append({
                    'RMA': rma_value, 'S/N': row.get('S/N', 'N/A'), 'SPC Code': row.get('SPC Code', 'N/A'), # Added SPC Code
...
def identify_overdue_for_shipping(df, days_threshold=1):
    required_cols = ['QA Approved Time', 'Estimate Complete', 'Estimate Approved', 'QA Approved', 'Shipped', 'RMA', 'S/N', 'SPC Code'] # Added SPC Code
    if df.empty or not all(col in df.columns for col in required_cols): return pd.DataFrame()
...
def identify_overdue_reminders(df, days_threshold=2):
    required_cols = ['Estimate Sent Time', 'Estimate Sent To Email', 'Reminder Completed', 'RMA', 'S/N', 'SPC Code', 'Reminder Contact Method'] # Added SPC Code
    if df.empty or not all(col in df.columns for col in required_cols): return pd.DataFrame()
    ...
    try:
        ...
        try:
            ...
            if archive_ws.row_count == 0 or archive_ws.row_values(1) != ARCHIVE_SHEET_HEADERS:
                archive_ws.clear(); archive_ws.append_row(ARCHIVE_SHEET_HEADERS)
...
def create_excel_report_bytes(report_data):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer: 
        ...
        for sheet_name_key, data_list in sheets_data.items():
            ...
            # Ensure SPC Code is present if data exists, even if all values are N/A for it
            if not df_report_sheet.empty and 'SPC Code' not in df_report_sheet.columns:
                df_report_sheet['SPC Code'] = 'N/A'
            
            # Reorder columns for consistency if needed, e.g., RMA, S/N, SPC Code, then others
            if not df_report_sheet.empty:
                cols_order = ['RMA', 'S/N', 'SPC Code']
                other_cols = [col for col in df_report_sheet.columns if col not in cols_order]
                final_cols = cols_order + other_cols
                # Ensure only existing columns are selected to prevent KeyErrors
                final_cols = [col for col in final_cols if col in df_report_sheet.columns]
                df_report_sheet = df_report_sheet[final_cols]
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    # todays_tasks_for_eod_report and todays_tasks_date should persist unless EOD logic itself changes them
    st.rerun() 
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
             ...
             # Ensure today's tasks for EOD are set if not already
            ...
            while current_date_to_report <= today:
                ...
                if current_date_to_report == today: # Capture tasks for EOD report basis
                    if st.session_state.get('todays_tasks_date') != today or st.session_state.get('todays_tasks_for_eod_report') is None:
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    # Fetch today's report from the ARCHIVE to get the definitive list of tasks for the day
    todays_archived_report_for_eod = None
    today_str = date.today().strftime("%Y-%m-%d")
    for rpt in archived_reports_gsheet: # Use the loaded archive
    ...
    if todays_archived_report_for_eod is None:
        ...
        summary_data = {"date": today_str, "estimate_tasks": [], "shipping_tasks": [], "reminder_tasks": []} 
        
        tasks_from_daily_report = todays_archived_report_for_eod # Use tasks from the archived daily report
        ...
        for task_type, task_list_key, status_col, completion_value, task_desc_template_base in [
            ...
            for task in tasks_from_daily_report.get(task_list_key, []): # Iterate over tasks from ARCHIVED daily report
                rma, sn = task.get("RMA"), task.get("S/N")
                spc_code = task.get("SPC Code", "N/A") # Get SPC Code from task if available
                ...
                if task_type == "estimate_tasks": task_description_content = f"Est. Complete: {task.get('Est. Complete Date', 'N/A')}"
                elif task_type == "reminder_tasks": task_description_content = f"Est. Sent: {task.get('Estimate Sent Time', 'N/A')}"
                elif task_type == "shipping_tasks": task_description_content = f"QA'd on {tasks_from_daily_report.get('date', 'N/A')}"
                
                task_description = f"{task_desc_template_base.split('{')[0] if '{' in task_desc_template_base else task_desc_template_base}{task_description_content})" if task_desc_template_base else f"Task for RMA {rma}, S/N {sn}"
...
if st.session_state.get('end_of_day_summary_report'): 
    ...
    if eod_summary['estimate_tasks']: 
        ...
        # Ensure all display columns are present
    ...
    if eod_summary.get('reminder_tasks'): 
        ...
        for col in eod_display_cols: # Ensure all display columns are present
    ...
    with pd.ExcelWriter(eod_output, engine='xlsxwriter') as writer:
        ...
        for sheet_name_key, data_list in eod_sheets_data.items():
            ...
            # Ensure all display columns are present for Excel export
            if not df_eod_sheet.empty:
                for col in eod_display_cols: # Use the defined EOD display columns
                    if col not in df_eod_sheet.columns: df_eod_sheet[col] = "N/A"
                df_eod_sheet = df_eod_sheet[eod_display_cols] # Order them
...
if not data_df.empty:
    ...
    if not overdue_estimates_df.empty:
        ...
        overdue_estimates_display_cols = ['RMA', 'S/N', 'SPC Code', 'Estimate Complete Time', 'Days Pending Approval', # Added SPC Code
                                          'Estimate Sent To Email', 'Estimate Sent Time', 
    ...
    if not overdue_reminders_df.empty:
        ...
        overdue_reminders_display_cols = ['RMA', 'S/N', 'SPC Code', 'Estimate Sent To Email', 'Estimate Sent Time', 'Days Pending Reminder', 'Reminder Contact Method', BC_LINK_COL_NAME] # Added SPC Code
    ...
    if not overdue_shipping_df.empty:
        ...
        overdue_shipping_display_cols = ['RMA', 'S/N', 'SPC Code', 'QA Approved Time', 'Days Pending Shipping', BC_LINK_COL_NAME] # Added SPC Code
