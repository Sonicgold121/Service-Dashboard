...
def generate_single_day_report_content(df, report_date_obj):
    ...
    shipping_df = df[
        ...
        (pd.to_datetime(df['QA Approved Time'], errors='coerce').dt.date == report_date_obj) ] # Ensure QA Approved Time is date for comparison
    ...
    estimate_df = df[
        ...
        (pd.to_datetime(df['Estimate Complete Time'], errors='coerce').dt.date == day_prior_to_report) ] # Ensure Estimate Complete Time is date
...
def create_excel_report_bytes(report_data):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer: # Use xlsxwriter engine
        ...
        # Sheet 1: Needs Estimate Creation
        ...
        if not df_needs_estimate.empty:
            ...
            worksheet1.set_row(0, 30) # Set title row height
            # Write headers with format
            for col_num, value in enumerate(df_needs_estimate.columns.values):
                worksheet1.write(2, col_num, value, header_format)
            # Apply cell format and set column widths
            for row_num in range(3, len(df_needs_estimate) + 3): # Data starts at excel row 3 (0-indexed) + header
                for col_num in range(len(df_needs_estimate.columns)):
                    worksheet1.write(row_num, col_num, df_needs_estimate.iloc[row_num-3, col_num], cell_format) # -3 because of title and header offset
            for i, col in enumerate(df_needs_estimate.columns):
                column_len = max(df_needs_estimate[col].astype(str).map(len).max(), len(col)) + 2
                worksheet1.set_column(i, i, column_len)
        else: 
            worksheet1 = writer.book.add_worksheet(sheet1_name) # Add empty sheet if no data
        ...
        if not df_needs_shipping.empty:
            ...
            worksheet2.set_row(0, 30) # Set title row height
            # Write headers with format
            for col_num, value in enumerate(df_needs_shipping.columns.values):
            ...
            for i, col in enumerate(df_needs_shipping.columns):
                column_len = max(df_needs_shipping[col].astype(str).map(len).max(), len(col)) + 2
                worksheet2.set_column(i, i, column_len)
        else:
            worksheet2 = writer.book.add_worksheet(sheet2_name) # Add empty sheet if no data
        ...
        if available_months:
            ...
            if reports_to_list:
                for i, report_data_item in enumerate(reports_to_list): 
                    ...
                    with col2:
                        if st.button("View/Download Report", key=f"view_archive_{report_data_item['date']}_{i}"): # Changed button label
