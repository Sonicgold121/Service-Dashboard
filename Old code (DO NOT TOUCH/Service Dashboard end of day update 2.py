...
def save_report_to_gsheet_archive(report_data, archive_sheet_name_to_save, archive_headers_to_check):
    ...
    try:
        ...
        if report_data['date'] in existing_dates and archive_sheet_name_to_save == ARCHIVE_SHEET_NAME: # Only check for duplicates for daily status, EOD can be overwritten
            return False 
        ...
        if archive_sheet_name_to_save == ARCHIVE_SHEET_NAME:
            row_to_append.extend([
             ...
             row_to_append.extend([
                ...
                json.dumps(report_data.get('adhoc_shipped_today', [])) # Add adhoc shipped
            ])
        
        archive_ws.append_row(row_to_append) 
        if archive_sheet_name_to_save == ARCHIVE_SHEET_NAME: get_archived_reports_from_gsheet.clear() # Clear daily report cache
        else: get_eod_summary_archive_from_gsheet.clear() # Clear EOD summary cache
        return True
...
def create_excel_report_bytes(report_data, report_type="Daily"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer: 
        ...
        for sheet_name_key, data_list in sheets_data.items():
            ...
            elif "Reminder" in sheet_name_key: default_cols.extend(['Estimate Sent To Email', 'Estimate Sent Time'])
            elif "Shipping" in sheet_name_key and report_type != "EOD" : pass # No extra for daily shipping
            elif "EOD" in sheet_name_key: default_cols.extend(['Status', 'Original Task'])
...
# EOD Summary Specific State
if 'eod_summary_archive' not in st.session_state: st.session_state.eod_summary_archive = [] # For session-based EOD archive display
...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    st.session_state.selected_eod_summary_to_display = None # Clear this too
    st.rerun() 
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    for rpt in archived_daily_reports_gsheet: # Check against daily report archive
    ...
    if todays_archived_report_for_eod is None:
        ...
        # Filter out items already on today's "needs_shipping" list for EOD
        ...
        for _, row in adhoc_shipped_df.iterrows():
            rma_str = str(row['RMA']).strip().lower()
            sn_str = str(row['S/N']).strip().lower()
            if (rma_str, sn_str) not in daily_shipping_rmas_sns: # Only add if not part of today's daily shipping tasks
        ...
        # Save EOD summary to its own archive
        if save_report_to_gsheet_archive(summary_data, EOD_SUMMARY_ARCHIVE_SHEET_NAME, EOD_ARCHIVE_SHEET_HEADERS):
            ...
            st.sidebar.warning("End of Day Summary Generated (already exists in archive or error saving).")
        st.rerun()
...
if st.session_state.get('end_of_day_summary_report'): 
    ...
    st.markdown("**Ad-hoc Shipped Today (not on initial daily report):**") # New Section
    if eod_summary.get('adhoc_shipped_today'):
        eod_adhoc_df = pd.DataFrame(eod_summary['adhoc_shipped_today'])
        for col in adhoc_shipped_cols: # Use specific columns for this section
    ...
    with pd.ExcelWriter(eod_output, engine='xlsxwriter') as writer:
        ...
        eod_sheets_data = { 
            ...
            "EOD AdHoc Shipped": eod_summary.get("adhoc_shipped_today", []) # New Sheet
        }
...
if not data_df.empty:
    ...
    # --- NEW: EOD Summary Archive Display ---
                ...
                for i, eod_summary_item in enumerate(eod_summaries_to_list):
                    ...
                    with col2:
                        if st.button("View/Download EOD Summary", key=f"view_archive_eod_{eod_summary_item['date']}_{i}"):
                            st.session_state.selected_eod_summary_to_display = eod_summary_item # New state variable
                            st.session_state.newly_generated_reports_to_display = []
                            st.session_state.selected_archived_report_to_display = None
                            st.session_state.custom_report_to_display = None
                            st.session_state.end_of_day_summary_report = None # Clear any live EOD summary
                            st.rerun()
