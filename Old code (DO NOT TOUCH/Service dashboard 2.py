    ...
    try:
        ...
        for col in string_cols_for_na_fill:
            if col in df.columns:
                ...
                # Changed from inplace=True to direct assignment to address FutureWarning
                df[col] = df[col].replace(['', 'nan', 'None', 'NaN', 'NONE'], 'N/A') 
        ...
        # At this point:
        # - String-intended columns have "N/A" for missing/empty values.
        # - Date columns have NaT for missing/unparseable dates.
        # - Other columns (e.g., purely numeric, if any) would retain their original NaN if they had them.
        # The final display `st.dataframe(filtered_df.astype(str))` will convert NaT to "NaT"
        # and any remaining actual pd.NaN to "nan" for display.
...
if not data_df.empty:
    ...
    for display_name, col_name in date_filter_columns.items():
        if col_name in filtered_df.columns:
            ...
            # The load_data function already attempts this conversion.
            # temp_col_dt = pd.to_datetime(filtered_df[col_name], errors='coerce') # Already done in load_data
