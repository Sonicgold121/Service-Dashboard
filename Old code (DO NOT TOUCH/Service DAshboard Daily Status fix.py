...
def generate_single_day_report_content(df, report_date_obj):
    ...
    shipping_df = df[
        (df['Estimate Complete'].astype(str).str.lower() == 'yes') & # Added condition
        (df['Estimate Approved'].astype(str).str.lower() == 'yes') & # Added condition
...
if not data_df.empty:
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Approved', 'Estimate Sent To Email']):
        eligible_estimate_sent_df = data_df[ (data_df['Estimate Approved'].astype(str).str.lower() == 'yes') & (data_df['Estimate Sent To Email'].astype(str).str.lower() == 'n/a') ]
