...
def identify_overdue_estimates(df, days_threshold=3):
    ...
    for _, row in df_copy.iterrows():
        ...
        if is_estimate_complete and is_not_approved and is_not_shipped and pd.notna(complete_time): 
            if (now - complete_time).days > days_threshold:
                ...
                overdue_items.append({
                    ...
                    'Days Pending Approval': (now - complete_time).days,
...
def identify_overdue_reminders(df, days_threshold=2):
    required_cols = ['Estimate Sent Time', 'Estimate Sent To Email', 
                     'Reminder Completed', 'Estimate Approved', # Added Estimate Approved
                     'RMA', 'S/N', 'SPC Code', 'Reminder Contact Method'] 
    if df.empty or not all(col in df.columns for col in required_cols): return pd.DataFrame()
    ...
    for _, row in df_copy.iterrows():
        ...
        is_estimate_not_approved = str(row.get('Estimate Approved', 'N/A')).lower() in ['no', 'n/a'] # New condition
        estimate_sent_time = row['Estimate Sent Time']
        rma_value = str(row.get('RMA', 'N/A'))
        
        # Item is overdue for reminder if: estimate sent, reminder not done, AND estimate not yet approved
        if is_estimate_sent and is_reminder_not_done and is_estimate_not_approved and pd.notna(estimate_sent_time):
            ...
            if days_passed_reminder > days_threshold:
                ...
                overdue_reminder_items.append({
                    ...
                    'Reminder Contact Method': row.get('Reminder Contact Method', 'N/A'), 
                    'Estimate Approved': row.get('Estimate Approved', 'N/A'), # Optionally display this
                    BC_LINK_COL_NAME: bc_url  
                })
...
def generate_single_day_report_content(df, report_date_obj):
    ...
    reminder_df = df[
        ...
        (df['Estimate Approved'].astype(str).str.lower().isin(['no', 'n/a'])) & # Added this condition
...
def display_formatted_report(report_data, source="Newly Generated", report_key_suffix=""): # For Daily/Custom reports
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    # Fetch fresh archive for EOD check
...
# Display Live EOD Summary (if generated)
...
if not data_df.empty:
    ...
    if not overdue_estimates_df.empty:
        st.warning("The following estimates were completed more than 3 days ago and are still pending approval (and not shipped):") # Updated title
    ...
    else: st.success("✅ No estimates are currently overdue for approval beyond 3 days (that haven't been shipped).") # Updated title
    st.markdown("---")
    
    st.subheader("🗣️ Overdue Reminders Report (Estimate Sent > 2 Days, Reminder Not Done, Estimate Not Approved)") # Updated title
    overdue_reminders_df = identify_overdue_reminders(data_df, days_threshold=2)
    if not overdue_reminders_df.empty:
        st.info("The following items had estimates sent >2 days ago, are pending a reminder, and are still awaiting approval:") # Updated title
        ...
        st.success("✅ No items are currently overdue for a reminder (that are still pending approval).") # Updated title
    st.markdown("---")
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Sent To Email', 'Reminder Completed', 'Estimate Approved']): # Ensure Estimate Approved is available
        eligible_reminder_df = data_df[ 
            (data_df['Estimate Sent To Email'].astype(str).str.lower() != 'n/a') & 
            (data_df['Reminder Completed'].astype(str).str.lower().isin(['no', 'n/a'])) &
            (data_df['Estimate Approved'].astype(str).str.lower().isin(['no', 'n/a'])) # Added this condition
        ]
        ...
        else: st.sidebar.info("No items currently eligible for reminder logging (either reminder done, estimate approved, or estimate not sent).")
