...
# Initialize session state for first load flags
...
if 'data_df' not in st.session_state:
    ...
    st.session_state.first_load_complete = True # Mark first load as complete

if st.button("🔄 Refresh Data from Google Sheet"):
    load_data_from_google_sheet.clear() 
    st.session_state.data_df = load_data_from_google_sheet()
    st.session_state.first_load_complete = False # Reset flag to re-apply default filters
    st.rerun() 
...
if not data_df.empty:
    ...
    for display_name, col_name in status_columns_to_filter.items():
        if col_name in filtered_df.columns:
            unique_values = ['All'] + sorted(list(set(val for val in filtered_df[col_name].astype(str).unique() if val))) # Ensure unique and remove potential empty strings from list
            
            default_index = 0 # Default to "All"
            if not st.session_state.first_load_complete and col_name == "Shipped":
                 if "N/A" in unique_values:
                     default_index = unique_values.index("N/A")
                 # If "N/A" is not an option but we want to default to it, we might need to add it or reconsider
                 # For now, if "N/A" is present, it will be selected on first load. Otherwise, "All".
    ...
    for display_name, col_name in date_filter_columns_to_filter.items():
        ...
        # Determine min/max for widget bounds from original data_df
        ...
        if can_setup_widget:
            # Set default value to None for date_input on first load
            default_date_value = None
            if st.session_state.first_load_complete: # If not first load, retain current selection (or full range if no selection yet)
                 # This part is tricky with st.date_input behavior.
                 # For simplicity, we'll make it always default to blank on refresh too, unless user interacts.
                 # To retain user selection across non-data-refresh reruns, session state for each date filter would be needed.
                 # For now, clicking refresh button will clear date selections.
                 pass # Default value remains None if first_load_complete is False

            current_date_range_selection = st.sidebar.date_input(
                f"Filter by {display_name}", 
                value=default_date_value, # Set to None to be blank initially
                min_value=min_val_for_widget_setup, 
                max_value=max_val_for_widget_setup, 
                key=f"date_range_{col_name}"
            )
            
            # Apply filter only if a valid date range is selected
            if current_date_range_selection and \
               len(current_date_range_selection) == 2 and \
               current_date_range_selection[0] is not None and \
               current_date_range_selection[1] is not None:
    ...
    # After all filters are processed on the first load, mark it as no longer the first load for subsequent reruns
                    ...
                    if st.sidebar.button("Mark as Shipped", key="mark_shipped_button"):
                        if rma_to_update and sn_to_update and shipped_date_val:
                            ...
                            if success:
                                ...
                                st.session_state.first_load_complete = False # Reset for filter defaults on refresh
