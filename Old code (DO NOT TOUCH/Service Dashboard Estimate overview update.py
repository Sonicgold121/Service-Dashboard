...
if not data_df.empty:
    ...
    if not overdue_df.empty:
        ...
        # Configure the overdue_df display with clickable link
        overdue_display_cols = ['RMA', 'S/N', 'Estimate Complete Time', 'Days Pending Approval', BC_LINK_COL_NAME]
        # Ensure BC_LINK_COL_NAME is present in overdue_df before trying to configure it
        if BC_LINK_COL_NAME not in overdue_df.columns:
            # This case should ideally not happen if identify_overdue_estimates works correctly
            # But as a fallback, create an empty column or handle gracefully
            overdue_df[BC_LINK_COL_NAME] = None 
            
        st.dataframe(
            overdue_df[overdue_display_cols], 
            ...
            # Define column order for display if needed, ensuring BC_LINK_COL_NAME is included
            column_order=overdue_display_cols 
        )
    else:
