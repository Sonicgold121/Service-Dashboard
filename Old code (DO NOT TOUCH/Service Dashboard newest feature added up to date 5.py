...
if st.button("🔄 Refresh Data from Google Sheet"):
    ...
    # st.session_state.todays_tasks_for_eod_report = None # Keep this to allow EOD report generation without re-running daily
...
if st.sidebar.button("Generate Daily Status Report(s)", key=f"gen_daily_report_btn_{st.session_state.refresh_counter}"):
    ...
    if data_df.empty:
        ...
        if current_date_to_report > today:
             ...
             # Ensure today's tasks are captured if it's today and report was already generated
...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    load_data_from_google_sheet.clear() # Ensure fresh data for EOD check
    current_data_for_eod = load_data_from_google_sheet()

    # Fetch today's report from the archive to get the definitive list of tasks
    todays_archived_report = None
    today_str = date.today().strftime("%Y-%m-%d")
    for rpt in archived_reports_gsheet: # Use the loaded archive
        if rpt['date'] == today_str:
            todays_archived_report = rpt
            break
    
    if todays_archived_report is None:
        st.sidebar.warning("Today's Daily Status Report has not been generated and archived yet. Please generate it first.")
        st.session_state.end_of_day_summary_report = None
        ...
        for task_type, task_list_key, status_col, completion_value, task_desc_template_base in [
            ...
            ("shipping_tasks", "needs_shipping", "Shipped", "yes", "Ship Item (QA'd on " + todays_archived_report.get('date', 'N/A') + ")")
        ]:
            for task in todays_archived_report.get(task_list_key, []): # Iterate over tasks from ARCHIVED daily report
                ...
                elif task_type == "shipping_tasks": task_description_content = f"QA'd on {todays_archived_report.get('date', 'N/A')}"
