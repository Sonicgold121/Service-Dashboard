...
if st.sidebar.button("Generate End of Day Summary", key=f"gen_eod_summary_btn_{st.session_state.refresh_counter}"):
    ...
    if 'todays_tasks_for_eod_report' not in st.session_state or st.session_state.todays_tasks_for_eod_report is None:
        ...
        for task in todays_tasks.get("needs_estimate_creation", []):
            ...
            status = "Pending"
            if not record.empty and record.iloc[0]['Estimate Sent To Email'].lower() != 'n/a': status = "Completed"
        ...
        for task in todays_tasks.get("needs_shipping", []):
            ...
            record = current_data_for_eod[(current_data_for_eod['RMA'] == rma) & (current_data_for_eod['S/N'] == sn)]
            status = "Pending"
            if not record.empty and record.iloc[0]['Shipped'].lower() == 'yes': status = "Completed"
            summary_data["shipping_tasks"].append({"RMA": rma, "S/N": sn, "Status": status, "Original Task": f"Ship Item (QA'd on {todays_tasks['date']})"})
        for task in todays_tasks.get("needs_reminder", []): # New EOD check
            rma, sn = task.get("RMA"), task.get("S/N")
            record = current_data_for_eod[(current_data_for_eod['RMA'] == rma) & (current_data_for_eod['S/N'] == sn)]
            status = "Pending"
            if not record.empty and record.iloc[0]['Reminder Completed'].lower() == 'yes': status = "Completed"
