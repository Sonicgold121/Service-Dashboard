       ...
       st.session_state.get('todays_tasks_date') != date.today():
        ...
        for task_type, task_list_key, status_col, completion_value, task_desc_template in [
            ...
            for task in todays_tasks.get(task_list_key, []):
                ...
                # Robust record lookup in the *current* data
                ...
                # Use .get() for task dictionary to avoid KeyError if a key is unexpectedly missing
                task_description_content = ""
                if task_type == "estimate_tasks":
                    task_description_content = f"Est. Complete: {task.get('Est. Complete Date', 'N/A')}"
                elif task_type == "reminder_tasks":
                    task_description_content = f"Est. Sent: {task.get('Estimate Sent Time', 'N/A')}"
                elif task_type == "shipping_tasks":
                    task_description_content = f"QA'd on {todays_tasks.get('date', 'N/A')}"
                
                task_description = f"{task_desc_template.split('{')[0] if '{' in task_desc_template else task_desc_template}{task_description_content})" if task_desc_template else f"Task for RMA {rma}, S/N {sn}"
