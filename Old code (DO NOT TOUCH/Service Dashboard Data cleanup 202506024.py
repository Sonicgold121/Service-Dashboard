...
def generate_single_day_report_content(df, report_date_obj):
    ...
    # Needs Reminder (Estimate Sent 2 days before report_date_obj, Reminder Not Completed, AND Estimate NOT Approved)
...
if not data_df.empty:
    ...
    # ... Sidebar actions ...
    # Pre-filter for valid records to use in all action sections
    valid_records_df = data_df[
        (data_df['RMA'].astype(str).str.strip().str.lower().ne('n/a')) &
        (data_df['RMA'].astype(str).str.strip().ne('')) &
        (data_df['S/N'].astype(str).str.strip().str.lower().ne('n/a')) &
        (data_df['S/N'].astype(str).str.strip().ne(''))
    ].copy()
    ...
    st.sidebar.markdown("---")
    st.sidebar.header("🔧 Data Cleanup (Incomplete Records)")
    incomplete_records_df = data_df[
    ...
    if not incomplete_records_df.empty:
        options = ["Select incomplete record..."] + [f"RMA: {rma}, S/N: {sn}" for rma, sn in zip(incomplete_records_df['RMA'], incomplete_records_df['S/N'])]
        selected_incomplete_item = st.sidebar.selectbox("Item to Update", options, index=0, key=f"incomplete_item_selector_{st.session_state.refresh_counter}")
        
        if selected_incomplete_item and selected_incomplete_item != "Select incomplete record...":
            try:
                parts = selected_incomplete_item.replace("RMA:", "").replace("S/N:", "").split(',')
                rma_to_update_incomplete = parts[0].strip()
                sn_to_update_incomplete = parts[1].strip()

                st.sidebar.write(f"Updating: RMA '{rma_to_update_incomplete}', S/N '{sn_to_update_incomplete}'")
                if st.sidebar.button("Mark as Shipped (Cleanup)", key=f"cleanup_shipped_button_{st.session_state.refresh_counter}"):
                    success = update_shipped_status_in_gsheet(rma_to_update_incomplete, sn_to_update_incomplete, date.today())
                    if success:
                        load_data_from_google_sheet.clear(); st.session_state.data_df = load_data_from_google_sheet(); st.session_state.first_load_complete = False; st.session_state.refresh_counter +=1; st.sidebar.success("Cleanup: Item marked as shipped!"); st.rerun()
            except Exception as e:
                st.sidebar.error(f"Error parsing selection: {e}")
    else:
        st.sidebar.info("No incomplete records found to clean up.")
