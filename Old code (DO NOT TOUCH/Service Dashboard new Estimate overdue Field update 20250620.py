...
def identify_overdue_estimates(df, days_threshold=3):
    required_cols = ['Estimate Complete Time', 'Estimate Complete', 'Estimate Sent To Email', 'RMA', 'S/N', 'SPC Code', 
                     'Shipped'] # Simplified required cols for this specific check
    ...
    for _, row in df_copy.iterrows():
        ...
        is_not_sent = str(row.get('Estimate Sent To Email', 'N/A')).lower() == 'n/a' # Main condition
        complete_time = row['Estimate Complete Time']
        
        # Condition is to identify items stuck BEFORE being sent
...
def identify_overdue_reminders(df, days_threshold=2):
    ...
    for _, row in df_copy.iterrows():
        ...
        # --- MODIFICATION: The check for "Estimate Approved" is now removed from this IF condition ---
        is_estimate_not_approved = str(row.get('Estimate Approved', 'N/A')).lower() in ['no', 'n/a'] # Kept for clarity but not used in main IF
...
if not data_df.empty:
    ...
    if all(c in data_df.columns for c in ['RMA', 'S/N', 'Estimate Sent To Email', 'Reminder Completed']):
        eligible_reminder_df = data_df[ (data_df['Estimate Sent To Email'].astype(str).str.lower() != 'n/a') & (data_df['Reminder Completed'].astype(str).str.lower().isin(['no', 'n/a'])) ] # Removed check for Estimate Approved
