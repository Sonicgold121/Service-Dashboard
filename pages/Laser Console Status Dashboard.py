...
# --- Page Configuration ---
st.set_page_config(
    page_title="Laser Console Status Dashboard", 
    page_icon="💡", 
    layout="wide",
)
...
# --- Main Application ---
st.title("Laser Console Approval Status")
st.markdown("This page tracks the status for all Lasers, Probes, and G6 items.")

# Define the part number prefixes to search for
part_prefixes = ('CUST-CYCLO-G6', 'CUST-IQ', 'CUST-SLA', 'CUST-SLX', 'CUST-SL', 'CUST-GLX', 'CUST-GL')
...
if data_df.empty:
    ...
    # Use str.startswith with the tuple of prefixes. na=False handles any empty Part Number cells.
    filtered_df = data_df[data_df['Part Number'].str.startswith(part_prefixes, na=False)].copy()

    if filtered_df.empty:
        st.info("✅ No service records found for the specified Laser Console and Probe parts.")
    else:
        ...
        with btn_col:
            st.write("") 
            st.download_button(
                label="📄 Download Laser/Probe Report",
                data=report_bytes,
                file_name=f"Laser_Probe_Action_Report_{date.today().strftime('%Y-%m-%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
        st.markdown("---")

        # The 'Part Number' column is added to the tables to differentiate items
        st.subheader("Estimates Not Approved")
        if not needs_approval_df.empty:
            display_cols_not_approved = ['RMA', BC_LINK_COL_NAME, 'Part Number', 'S/N', 'Description', 'Fault Comments', 'Sender', 'Estimate Complete Time']
            ...
            st.success("✅ All Laser/Probe estimates have an approval status.")

        st.markdown("---")

        st.subheader("Estimate Approved")
        if not awaiting_qa_df.empty:
            display_cols_approved = ['RMA', BC_LINK_COL_NAME, 'Part Number', 'S/N', 'Description', 'Sender', 'Estimate Approved Time', 'Days Since Approval']
            ...
            st.success("✅ No Laser/Probe items are currently awaiting QA.")