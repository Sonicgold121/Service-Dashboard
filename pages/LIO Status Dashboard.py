...
# --- Page Configuration ---
st.set_page_config(
    page_title="LIO Status Dashboard", 
    page_icon="📋", 
    layout="wide",
)
...
def create_cust_lio_excel_report(awaiting_qa_df, needs_approval_df):
...
# --- Main Application ---
st.title("LIO Estimate Approval Status")
st.markdown("This page tracks the approval and QA status for all service items with the part number `CUST-LIO`.")
...
if data_df.empty:
    ...
    # Filter for the specific part number
    cust_lio_df = data_df[data_df['Part Number'] == 'CUST-LIO'].copy()

    if cust_lio_df.empty:
        st.info("✅ No service records found with the part number 'CUST-LIO'.")
    else:
        # Add the Business Central link column
        cust_lio_df[BC_LINK_COL_NAME] = cust_lio_df.apply(
        ...
        # Split into "needs approval" and "awaiting QA" dataframes
        needs_approval_df = cust_lio_df[cust_lio_df['Estimate Approved'].str.lower() == 'no']
        
        # NEW LOGIC: Appears here if Estimate Approved is 'Yes' AND QA Approved is 'No'
        awaiting_qa_df = cust_lio_df[
            (cust_lio_df['Estimate Approved'].str.lower() == 'yes') & 
            (cust_lio_df['QA Approved'].str.lower() == 'no')
        ].copy()

        # --- Add "Days Since Approval" and sort the awaiting QA table ---
        if not awaiting_qa_df.empty:
        ...
        # --- Display KPIs and Download Button ---
        st.markdown("---")
        kpi1, kpi2, btn_col = st.columns([1, 1, 2])
        kpi1.metric(
            label="Waiting for Approval",
            value=len(needs_approval_df)
        )
        kpi2.metric(
            label="Approved & Awaiting QA",
            value=len(awaiting_qa_df)
        )
        
        # Prepare data for download
        report_bytes = create_cust_lio_excel_report(awaiting_qa_df, needs_approval_df)
        with btn_col:
            st.write("") # Spacer
            st.download_button(
                label="📄 Download Report (Awaiting QA & Approval)",
                data=report_bytes,
                file_name=f"CUST_LIO_Action_Report_{date.today().strftime('%Y-%m-%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
        st.markdown("---")

        # --- Display "Needs Approval" Table ---
        st.subheader("Estimates Not Approved")
        if not needs_approval_df.empty:
            display_cols_not_approved = [
                'RMA', BC_LINK_COL_NAME, 'S/N', 'Description', 
                'Fault Comments', 'Sender', 'Estimate Complete Time', 'Estimate Sent Time'
            ]
            st.dataframe(
                needs_approval_df[display_cols_not_approved], 
                use_container_width=True,
                column_config={
            ...
            st.success("✅ All CUST-LIO estimates have an approval status.")

        st.markdown("---")

        # --- Display "Awaiting QA" Table ---
        st.subheader("Estimate Approved")
        if not awaiting_qa_df.empty:
            display_cols_approved = [
                'RMA', BC_LINK_COL_NAME, 'S/N', 'Description', 
                'Sender', 'Estimate Approved Time', 'Days Since Approval'
            ]
            st.dataframe(
                awaiting_qa_df[display_cols_approved], 
                use_container_width=True,
                column_config={
                    BC_LINK_COL_NAME: st.column_config.LinkColumn(
                        label="Business Central", 
                    ...
                    'Estimate Approved Time': st.column_config.DateColumn(
                        "Approved Date",
                        format="YYYY-MM-DD"
                    )
                }
            ...
            st.success("✅ No CUST-LIO items are currently awaiting QA.")